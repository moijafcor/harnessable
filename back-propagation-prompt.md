...
find ~/code -name "HARNESSABLE_VERSION" \
    \( -path "*/vendor/harnessable/HARNESSABLE_VERSION" \
       -o -path "*/.agents/skills/harnessable/HARNESSABLE_VERSION" \) \
    -not -path "*/harnessable/framework/*" \
    2>/dev/null | while read f; do
  if echo "$f" | grep -q "/.agents/skills/"; then
    PROJECT=${f%/.agents/skills/harnessable/HARNESSABLE_VERSION}; KIND=codex
  else
    PROJECT=${f%/docs/harness/vendor/harnessable/HARNESSABLE_VERSION}; KIND=full
  fi
  PINNED=$(cat $f)
  CURRENT=$(git -C $FRAMEWORK_ROOT rev-parse --short HEAD)
  echo "PROJECT=$PROJECT | kind=$KIND | pinned=$PINNED | current=$CURRENT"
done
```
## Step 2 — Sync each installation
For each project, choose by discovered `kind`:
```sh
# kind=full
bash $FRAMEWORK_ROOT/install.sh --update $PROJECT_ROOT
# kind=codex
bash $FRAMEWORK_ROOT/codex/install.sh --update $PROJECT_ROOT
```

Complete one project before moving to the next.
Do not proceed if a project has a dirty working tree —
`install.sh --update` and `codex/install.sh --update` will detect and halt.
...
```
## Final report
Report each project with result, MERGE needed, and REPLACE remaining.
